...
<div class="container" id="contentBox" style="position: relative;">
		...
		<div class="row">
			<div class="col-md-4 col-md-offset-8">
				<button class="btn btn-success" id="dept_add_modal_btn"><span class="glyphicon glyphicon-plus"></span>新增院系</button>
			</div>
		</div>
		<!-- 院系修改模态框 -->
		<div class="modal fade" id="deptUpdateModal" tabindex="-1" role="dialog">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">院系修改</h4>
				</div>
				<div class="modal-body">
					<form class="form-horizontal">
						<!-- <div class="form-group">
							<label class="col-sm-2 control-label">院系编码</label>
							<div class="col-sm-10">
								<input type="text" name="deptId" class="form-control" id="deptId_update_input"
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" name="deptId" class="form-control"
									id="deptId_add_input" placeholder="院系编码"> <span
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" name="deptName" class="form-control"
									id="dept_add_input" placeholder="院系名称"> <span
									class="help-block"></span>
							</div>
						</div>						
					</form>
...
<script type="text/javascript">
	...
	function build_dept_table(result) {
		...
		dept.forEach(function(obj,i){
			str +='<tr><td>'+obj.deptId
			+'</td><td><span style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;display:inline-block;width:200px;" title="'+obj.deptName+'">'+obj.deptName			
			+'</td><td><button edit-id='+obj.deptId+' class="btn btn-primary edit_btn btn-xs"><span class="glyphicon glyphicon-pencil"></span>修改</button><button del-id='+obj.deptId+' class="btn btn-danger delete_btn btn-xs"><span class="glyphicon glyphicon-trash"></span>删除</button></td></tr>';			
		})
		$("#dept_tbody").empty().append(str);
	}
	...
	function getDept(deptId) {
		$.ajax({
			url : "${APP}/getDeptBydeptId/" + deptId,
			type : "GET",
			success : function(result) {
				console.log(result);
				var department = result.extend.department;
				$("#deptId_update_input").val(department.deptId);
				$("#deptName_update_input").val(department.deptName);
			}
		})
	}
	// 五五五    点击更新院系信息
	$("#dept_update_btn").click(function() {
		//发送请求  保存员工数据
		var hello=$("#deptUpdateModal form").serialize();
		console.log(hello);
		$.ajax({
			url : "${APP}/saveDepartmentBydeptId/" + $(this).attr("edit-id"),
			type : "PUT",
	...
	$(document).on("click", ".delete_btn", function() {
		...
		if (confirm("确认删除【" + deptName + "】吗？")) {
			//确认发送请求删除
			$.ajax({
				url : "${APP}/deleteDepartment/" + deptId,
				type : "DELETE",
				success : function(result) {
					//alert(result.msg);
					to_page(currentPage);
